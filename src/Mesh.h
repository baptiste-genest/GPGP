...
using ManifoldSurfaceMesh = geometrycentral::surface::ManifoldSurfaceMesh;
...
struct Mesh {
    ...
    Mesh(std::string file) {
        std::tie(topology, geometry) = geometrycentral::surface::readManifoldSurfaceMesh(file);
    }
    ...
    scalars dualAreas() const {
        scalars A(topology->nVertices(),0);
        for (auto v : topology->vertices())
            A[v.getIndex()] = geometry->vertexDualArea(v);
        return A;
    }
        ...
        vec nor = cross( ba, ac );
          ...
          return std::sqrt(
            (sign(dot(cross(ba,nor),pa)) +
             ...
             ?
             std::min( std::min(
             dot2(ba*std::clamp<scalar>(dot(ba,pa)/dot2(ba),0.0,1.0)-pa),
             dot2(cb*std::clamp<scalar>(dot(cb,pb)/dot2(cb),0.0,1.0)-pb) ),
             dot2(ac*std::clamp<scalar>(dot(ac,pc)/dot2(ac),0.0,1.0)-pc) )
             :
    ...
    static Vec proj_b_along_a(const Vec& x,const Vec& y) {
        return x - x.dot(y)*y;
    }
    ...
    scalar diameter(Face f) const {
        ...
        for (auto e : f.adjacentEdges())
            M = std::max(M,(scalar)geometry->edgeLength(e));
        return M;
    }
    ...
    */
    ...
    void normalize(scalar f = 1) {
        // normlize vertex positions to fit between [-1,1]^3
        geometry->requireVertexPositions();
        bary = vec::Zero();
        for (const auto& v : topology->vertices())
            bary += toVec(geometry->vertexPositions[v]);
        bary /= topology->nVertices();
        // compute bouding box to scale the mesh
        vec min = bary, max = bary;
        for (const auto& v : topology->vertices()) {
            auto pos = toVec(geometry->vertexPositions[v]);
            min = min.cwiseMin(pos);
            max = max.cwiseMax(pos);
        }
        vec center = (min + max) / 2;
        vec scale = max - min;
        scalar l = f/scale.maxCoeff();
        for (const auto& v : topology->vertices()) {
            auto pos = toVec(geometry->vertexPositions[v]);
            pos = (pos - center) * l;
            geometry->vertexPositions[v] = toVector3(pos);
        }
    ...
    Vertex randomVertex() const {
        return topology->vertex(rand() % topology->nVertices());
    }
};

inline Mesh Soupify(const Mesh& M,scalar noise) {
    Mesh S;
    ...
    for (auto f : M.topology->faces()) {
        ...
        for (auto v : f.adjacentVertices()){
            F[i++] = c++;
            positions.push_back(M.geometry->vertexPositions[v]);
            for (auto i : range(3))
                positions.back()[i] += sampleUnitGaussian(1)[0]*noise;
        }
        faces.push_back(F);
    }

    std::tie(S.topology,S.geometry) = geometrycentral::surface::makeManifoldSurfaceMeshAndGeometry(faces,positions);
